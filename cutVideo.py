...
class CutVideo(object):
    ...
    def cut_video(self,file_name, file_video_save, file_atual,corte,x):
        for i in range(len(corte)-1):
            ...
            os.system("ffmpeg -i " + str(file_atual) + "/" + str(file_name) + " -ss " + str(inicio) + " -t " + str(duracao) + " -acodec libvorbis -vcodec libtheora -ar 22050 " + str(file_video_save) +str(x)+ "_part_" + str(i + 1) + ".ogg")      
            time.sleep(1)       
